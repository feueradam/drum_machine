...
(function(MIDI) { 'use strict';
	...
	window.AudioContext && (function() {
		...
		midi.noteOn = function(channelId, noteId, velocity, delay, panning) {
			delay = delay || 0;
			...
			if (buffer) {
				...
				if (panning){
					const panner = new StereoPannerNode(ctx, { pan: panning });
					source.connect(panner).connect(ctx.destination);
					source.gainNode.connect(panner).connect(ctx.destination);
				} else {
